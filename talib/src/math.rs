...
pub fn ta_add(
    ...
    let lookback = unsafe { TA_ADD_Lookback() };
...
pub fn ta_div(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_DIV_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_DIV(
            0,
...
pub fn ta_max(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_MAX_Lookback(kwargs.timeperiod) };
    let (mut out, ptr) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_MAX(
            0,
...
pub fn ta_maxindex(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_MAXINDEX_Lookback(kwargs.timeperiod) };
    let (mut out, ptr) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_MAXINDEX(
            0,
...
pub fn ta_min(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_MIN_Lookback(kwargs.timeperiod) };
    let (mut out, ptr) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_MIN(
            0,
...
pub fn ta_minindex(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_MININDEX_Lookback(kwargs.timeperiod) };
    let (mut out, ptr) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_MININDEX(
            0,
...
pub fn ta_minmax(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_MINMAX_Lookback(kwargs.timeperiod) };
    ...
    match ret_code {
        TA_RetCode::TA_SUCCESS => {
            if out_size != 0 {
                unsafe {
                    out_min.set_len(out_size);
                }
                unsafe {
                    out_max.set_len(out_size);
                }
            } else {
                unsafe {
                    out_min.set_len(len);
...
pub fn ta_minmaxindex(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_MINMAXINDEX_Lookback(kwargs.timeperiod) };
    let (mut out_min, ptr_min) = make_vec(len, lookback);
    let (mut out_max, ptr_max) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_MINMAXINDEX(
...
pub fn ta_mult(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_MULT_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_MULT(
            0,
...
pub fn ta_sub(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_SUB_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_SUB(
            0,
...
pub fn ta_sum(
    ...
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let lookback = unsafe { TA_SUM_Lookback(kwargs.timeperiod) };
    let (mut out, ptr) = make_vec(len, lookback);
    let ret_code = unsafe {
        TA_SUM(
            0,
...
pub fn ta_acos(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_ACOS_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let ret_code = unsafe {
...
pub fn ta_asin(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_ASIN_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let ret_code = unsafe {
...
pub fn ta_atan(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_ATAN_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let ret_code = unsafe {
...
pub fn ta_ceil(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_CEIL_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let ret_code = unsafe {
...
pub fn ta_cos(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_COS_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let ret_code = unsafe {
...
pub fn ta_cosh(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_COSH_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
    let ret_code = unsafe {
...
pub fn ta_exp(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_EXP_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
...
pub fn ta_floor(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_FLOOR_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
...
pub fn ta_ln(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_LN_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
...
pub fn ta_log10(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_LOG10_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
...
pub fn ta_sin(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_SIN_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
...
pub fn ta_sinh(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_SINH_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
...
pub fn ta_sqrt(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_SQRT_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
...
pub fn ta_tan(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_TAN_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
...
pub fn ta_tanh(real_ptr: *const f64, len: usize) -> Result<Vec<f64>, TA_RetCode> {
    let lookback = unsafe { TA_TANH_Lookback() };
    let (mut out, ptr) = make_vec(len, lookback);
    let mut out_begin: TA_Integer = 0;
    let mut out_size: TA_Integer = 0;
